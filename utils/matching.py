...
def match_bipartite_greedy(weight_matrix):
    ...
    then out of the remaining ground truth boxes, the ground truth box that has the greatest similarity
    with any of the remaining anchor boxes will be matched second, and so on. That is, the ground truth
    boxes will be matched in descending order by maximum similarity with any of the respectively
    remaining anchor boxes.

    The runtime complexity is O(m^2 * n), where `m` is the number of ground truth boxes and `n` is
    the number of anchor boxes.

    Arguments:
       weight_matrix : A 2D Numpy array that represents the weight matrix
    ...
    # In each iteration of the loop, exactly one ground truth box will be matched to one anchor box.
    for _ in range(num_ground_truth_boxes):
        # Find the maximum anchor-ground truth pair in two steps:
        # First, reduce over the anchor boxes and then reduce over the ground truth boxes.
        anchor_indices = np.argmax(weight_matrix, axis=1)  # Reduce along the ground truth box axis.
        ...
        # Set the row of the matched ground truth box and the column of the matched anchor box to
        # all zeros. This ensures that those boxes will not be matched again, because they will
        # never be the best matches for any other boxes.
...
def match_multi(weight_matrix, threshold):
    """
    Match all elements along the second axis of `weight_matrix` to their best
    matches along the first axis subject to the constraint that the weight of
    a match must be greater than or equal to `threshold` in order to produce
    a match.

    If the weight matrix contains elements that should be ignored, the row or
    column representing the respective element should be set to a value below
    `threshold`.
    ...
    Arguments:
        weight_matrix: A 2D Numpy array that represents the weight matrix
            ...
            numbers. The matching process will maximize, i.e. larger weights are
            preferred over smaller weights.
        threshold: A float that represents the threshold (i.e. lower bound)
    ...
    anchor_indices_threshold_met = np.nonzero(overlaps >= threshold)[0]
    gt_indices_threshold_met = ground_truth_indices[anchor_indices_threshold_met]

    return gt_indices_threshold_met, anchor_indices_threshold_met
